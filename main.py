...
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QVBoxLayout, QSystemTrayIcon, QMenu, QAction, QDesktopWidget
from windowsgui import ChatWindow
...
class DesktopPet(QWidget):
    ...
    def initPetImage(self):
        ...
        self.movie = QMovie("Pom Pom Purin1.gif")
        # 设置标签大小
        ...
        # 对话框样式设计
        self.show_time_rest.setStyleSheet("font:15pt '微软雅黑';border-width: 1px;color:blue;")
        ...
        # 将宠物正常待机状态的动图放入pet1中
        self.pet1 = []
        for i in os.listdir("Pom Pom Purin"):
            self.pet1.append("Pom Pom Purin/" + i)
        # 将宠物正常待机状态的对话放入pet2中
        self.dialog = []
        # 读取目录下dialog文件
        with open("dialog.txt", "r", encoding="ANSI", errors="ignore") as f:
            text = f.read()
    ...
    def randomAct(self):
        ...
        # 这里可以通过else-if语句往下拓展做更多的交互功能
            ...
            # 把表情设定为固定的动作
            self.movie = QMovie("./click/rest.gif")
            # 宠物大小
            self.movie.setScaledSize(QSize(200, 200))
            # 将动画添加到label中
            self.image.setMovie(self.movie)
    ...
    def contextMenuEvent(self, event):
        ...
        question_answer = menu.addAction("AI聊天")
        if self.rest_open == 1:
    ...
    # 休息时间
    def haveRest(self):
        print(self.rest_open)
        if self.rest_open == 2:
            self.show_time_rest.setText("休息一下，喝点水吧~")
        ...
        self.rest_timer.timeout.connect(self.restoreCondition)
        self.rest_timer.start(5000)


    def restoreCondition(self):

        self.condition = 0
        self.show_time_rest.setText("1")
        self.randomAct()
